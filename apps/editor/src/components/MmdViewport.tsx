...
export function MmdViewport() {
  ...
  const actor = useMemo(() => project.actors.find((item) => item.actorId === selectedActorId) ?? project.actors[0], [project.actors, selectedActorId]);
  ...
  const activeMotion = evaluation.active.find((item) => item.track.type === 'motion' && item.track.actorId === actor?.actorId);
  ...
  useEffect(() => {
    ...
    setBusy(true);
    void runtime.loadModel(modelAsset.runtimeUrl, modelAsset.title).catch(() => { loadedModelRef.current = null; }).finally(() => setBusy(false));
